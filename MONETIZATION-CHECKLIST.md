...
- ✅【已完成】7 个广告位已创建（自适应尺寸，站点 fakebreak.win，在你自己的账号 #514478 下）：2452943 首页 / 2452944 烟盒页 / 2452945 解锁插页 / 2452946 结算页 / 2452947 拆盒页 / 2452948 隐私页 / 2452949 关于页；抽烟主界面刻意不放（遮按钮、毁沉浸感），
...
### 路线 B：激励广告（✅ 已用现行最优方案落地）

- ✅【已完成】"看广告解锁"插页已嵌入真实 A-Ads 广告位 2452945：
  用户每次看激励插页 = 一次真实付费展示，奖励仍由倒计时发放
  （展示广告政策禁止"点击广告换奖励"，计时解锁是合规做法）
- 真正的激励**视频**广告在网页端几乎没有合规入口：
  Google Ad Placement API（adBreak）仍是 closed beta 且要求 H5 游戏；
  AdMob/Unity 激励广告只面向 App；网页 Offerwall（Torox/AdGem 等）
  门槛低但充斥问卷/下载任务，体验脏、会拉低留存，**不建议接**
- 代码已预留切换口：`src/monetization.ts` 的 `AD_CONFIG.provider`
  改为 "network" 并填入 SDK 参数即可接入未来的激励视频网络，
  界面与解锁逻辑无需改动
